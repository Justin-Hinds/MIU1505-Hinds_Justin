...
- (NSString*)userAgent
{
	if (userAgent==nil)
	{
		...
		NSString *kk1505_AboutMe_Hinds_JustinUserAgentPrefix = [NSString stringWithFormat:@"%s%s%s %s%s","Appc","eler","ator","Tita","nium"];
		userAgent = [[NSString stringWithFormat:@"%@/%s (%@)",kk1505_AboutMe_Hinds_JustinUserAgentPrefix,TI_VERSION_STR,currentDeviceInfo] retain];
	}
	return userAgent;
}
...
+(NSDictionary *)tiAppProperties
{
    ...
    if(props == nil) {
        ...
        if(jsonData) {
            ...
            // If we have no data...
            // This should never happen on a k1505_AboutMe_Hinds_Justin app using the node.js CLI
