...
NSString * k1505_AboutMe_Hinds_Justin$ModuleRequireFormat = @"(function(exports){"
...
@implementation k1505_AboutMe_Hinds_JustinObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_k1505_aboutme_hinds_justin);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	[context gc];
	[_k1505_aboutme_hinds_justin gc];
}
...
{
	// create k1505_AboutMe_Hinds_Justin global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "k1505_AboutMe_Hinds_Justin" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_k1505_aboutme_hinds_justin = [[k1505_AboutMe_Hinds_JustinObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_k1505_aboutme_hinds_justin];
	
	NSString *_k1505_aboutme_hinds_justinNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _k1505_aboutme_hinds_justinNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_k1505_aboutme_hinds_justin valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:k1505_AboutMe_Hinds_Justin$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.k1505_aboutme_hinds_justin.kroll" 
