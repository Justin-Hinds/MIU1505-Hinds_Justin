...
- (NSString*)userAgent
{
	if (userAgent==nil)
	{
		...
		NSString *kk1505_UtilityApp_Justin_HindsUserAgentPrefix = [NSString stringWithFormat:@"%s%s%s %s%s","Appc","eler","ator","Tita","nium"];
		userAgent = [[NSString stringWithFormat:@"%@/%s (%@)",kk1505_UtilityApp_Justin_HindsUserAgentPrefix,TI_VERSION_STR,currentDeviceInfo] retain];
	}
	return userAgent;
}
...
+(NSDictionary *)tiAppProperties
{
    ...
    if(props == nil) {
        ...
        if(jsonData) {
            ...
            // If we have no data...
            // This should never happen on a k1505_UtilityApp_Justin_Hinds app using the node.js CLI
