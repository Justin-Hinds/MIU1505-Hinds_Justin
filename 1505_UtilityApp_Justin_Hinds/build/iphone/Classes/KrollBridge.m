...
NSString * k1505_UtilityApp_Justin_Hinds$ModuleRequireFormat = @"(function(exports){"
...
@implementation k1505_UtilityApp_Justin_HindsObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_k1505_utilityapp_justin_hinds);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	[context gc];
	[_k1505_utilityapp_justin_hinds gc];
}
...
{
	// create k1505_UtilityApp_Justin_Hinds global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "k1505_UtilityApp_Justin_Hinds" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_k1505_utilityapp_justin_hinds = [[k1505_UtilityApp_Justin_HindsObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_k1505_utilityapp_justin_hinds];
	
	NSString *_k1505_utilityapp_justin_hindsNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _k1505_utilityapp_justin_hindsNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_k1505_utilityapp_justin_hinds valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:k1505_UtilityApp_Justin_Hinds$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.k1505_utilityapp_justin_hinds.kroll" 
